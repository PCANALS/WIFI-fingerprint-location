...
plot_ly(type = "scatter3d", mode="markers",
        wifi_t_signal_val_e,
        color = ~error, colors = c("#ff0000", "#1289d7"),
        x=~LATITUDE, y=~LONGITUDE, z= ~FLOOR)

plot_ly(wifi_t_signal_val_e, type = "scatter3d", mode="markers", 
        colors = c("#ff0000","#1289d7"), sizes = 1)%>%
  add_markers(wifi_t_signal_val_e,
        color = ~error, 
        marker= list(size=4),
        x=~LATITUDE, y=~LONGITUDE, z= ~FLOOR)

wifi_t_user6<-wifi_train_o
wifi_t_user6$USERID<-wifi_t_user6$USERID

wifi_t_user6<-wifi_t_user6%>%dplyr::mutate(user = ifelse("6"%in%USERID, "USER6", "others"))


summary(wifi_t_user6$user)
unique(wifi_t_signal_6$user)

summary



wifi_t_signal_val_e$error
