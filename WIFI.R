...
summary(y[nowaps])#vemos el user a investigar#

wifi_t_signal%>%filter(USERID==14)%>%count()
...
# knn_B<-train(BUILDINGID~WAP_max_value,data=wifi_t_signal,
...
svm_l_lat<-train(LATITUDE~WAP_max,data=train, #aplicado en el train completo#
...
# 
# 
ggplot(wifi_t_signal, aes(x=BUILDINGID),)+
         geom_count(aes(y=WAP323), na.rm = FALSE)

ggplot(wifi_t_signal_val, aes(x=BUILDINGID),)+
  geom_count(aes(y=WAP323), na.rm = FALSE)
